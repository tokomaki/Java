...
public class WordNet {
    ...
    public WordNet(String synsets, String hypernyms) {
        // symbol table maps a synset noun to a set of synset IDs
        this.st = new RedBlackBST<String, SET<Integer>>();
        // symbol table maps a synset ID to corresponding synset string
        this.rst = new RedBlackBST<Integer, String>();
        // In object for synsets
        In synIn = new In(synsets);
        int id = 0;
        while (!synIn.isEmpty()) {
            String[] a = synIn.readLine().split(",");
            id = Integer.parseInt(a[0]);
            String[] synset = a[1].split("\\s");
            for (String noun : synset) {
                if (!st.contains(noun)) { st.put(noun, new SET<Integer>()); }
                st.get(noun).add(id);
            }
            rst.put(id, a[1]);
        }
    ...
    public String sca(String noun1, String noun2) {
        int common = sca.ancestor(st.get(noun1), st.get(noun2));
        return rst.get(common);
    }
